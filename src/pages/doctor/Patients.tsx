...
import { Plus, MoreVertical, X, User, Phone, Mail, MapPin, Heart, FileText, Trash2 } from 'lucide-react';
import { toast } from 'sonner';

const DoctorPatients: React.FC = () => {
  const { t } = useLanguage();
  const { user } = useAuth();
  const { getPatientsByDoctor, addPatient, deletePatient } = useData();
        ...
        <main className="flex-1 p-8">
          ...
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-muted-foreground mt-1">{patients.length} {t('patientsRegistered')}</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              <div key={patient.id} className="ayur-card relative group">
                  ...
                  {activeMenu === patient.id && (
                    <div className="absolute right-0 mt-2 w-48 bg-card rounded-lg shadow-lg border border-border z-20">
                      <button
                        ...
                        className="w-full px-4 py-3 text-left hover:bg-secondary/50 transition-colors rounded-t-lg flex items-center gap-2"
                      >
                      ...
                      <button
                        ...
                        className="w-full px-4 py-3 text-left hover:bg-destructive/10 transition-colors rounded-b-lg flex items-center gap-2 text-destructive"
                      >
                ...
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{patient.disease || t('noCondition')}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {patient.age > 0 && (
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span>{patient.age} {t('years')}, {patient.gender}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-border flex justify-between items-center">
                  <span className="text-xs text-muted-foreground">
                    {t('registered')}: {new Date(patient.registeredAt).toLocaleDateString()}
                  </span>
                  {patient.bloodGroup && (
            ...
            {patients.length === 0 && (
              <div className="col-span-full text-center py-16">
                ...
                <h3 className="text-xl font-semibold text-foreground mb-2">{t('noPatientsYet')}</h3>
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 rounded-lg bg-secondary/30">
                      ...
                      <p className="font-semibold">{selectedPatient.age || 'N/A'} {t('years')}</p>
                    </div>
                    <div className="p-4 rounded-lg bg-secondary/30">
                  ...
                  <div className="mt-4 p-4 rounded-lg bg-secondary/30">
                    ...
                    <p className="font-semibold">{selectedPatient.disease || t('notSpecified')}</p>
                  </div>

                  <div className="mt-4 p-4 rounded-lg bg-secondary/30">
                    <p className="text-sm text-muted-foreground mb-1">{t('symptoms')}</p>
                    <p className="font-semibold">{selectedPatient.symptoms || t('notSpecified')}</p>
                  </div>

                  <div className="mt-4 p-4 rounded-lg bg-secondary/30">
                    <p className="text-sm text-muted-foreground mb-1">{t('medicalHistory')}</p>
                    <p className="font-semibold">{selectedPatient.medicalHistory || t('notSpecified')}</p>
                  </div>

                  <div className="mt-4 p-4 rounded-lg bg-secondary/30">
                    <p className="text-sm text-muted-foreground mb-1">{t('address')}</p>
                    <p className="font-semibold">{selectedPatient.address || t('notSpecified')}</p>
                  </div>
                </div>
              </div>
            </div>
